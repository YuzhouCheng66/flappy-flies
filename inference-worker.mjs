...
async function request(type,body){
 if(type==='init'){
  ...
  FlyGPU.create(meta,p=>postMessage({event:'progress',...p})).then(gpu=>{engine.gpu=gpu;gpu.reset();ready=true;postMessage({event:'ready'});}).catch(e=>postMessage({event:'failed',error:e.message}));
 ...
 if(type==='step'){
  ...
  return{generation,frames,done:engine.done,msPerTick:(performance.now()-start)/Math.max(1,frames.length)};
 }
